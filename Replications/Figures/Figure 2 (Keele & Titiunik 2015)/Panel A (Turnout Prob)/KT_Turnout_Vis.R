...
probs <-  predict(cef, type="response")
pred_Y <- probs
pred_plot_probs <- data.frame(pred_Y, X1, X2)

pred_Y <- round(probs)
pred_plot_round <- data.frame(pred_Y, X1, X2)

bern <- function(prob) {
  return(as.numeric(rbernoulli(n=1,p=prob)))
}

pred_Y <- sapply(probs, FUN=bern)
pred_plot_coin <- data.frame(pred_Y, X1, X2)

p1 <- ggplot() + 
  geom_point(data = df_plot, aes(x=X2, y=X1, color=e2008g), size=0.8) +
  geom_path(aes(points$longitude, points$latitude), size=0.8) +
  scale_color_gradient2(low = 'dodgerblue3', midpoint=0.5, mid="lavender", high = 'firebrick3', limits=c(0, 1)) +
...
p2 <- ggplot() +
  geom_point(data = pred_plot_probs, aes(x=X2, y=X1, color = pred_Y), size=0.8) +
  geom_path(aes(points$longitude, points$latitude), size=0.8) +
  scale_color_gradient2(low = 'dodgerblue3', midpoint=0.5, mid="lavender", high = 'firebrick3', limits=c(0, 1)) +
  labs(title="Predicted Turnout Probability",
       x ="Longitude", y = "Latitude", color = "Turnout") + 
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+ 
  geom_point(data = points[c(30, 45, 60),], aes(x=longitude, y=latitude), shape = 15, color = "black", size = 2, show.legend = FALSE) +
...
# p3 <- ggplot() +
#   geom_point(data = pred_plot_round, aes(x=X2, y=X1, color = pred_Y), size=0.8) +
#   geom_path(aes(points$longitude, points$latitude), size=0.8) +
#   scale_color_gradient2(low = 'dodgerblue3', midpoint=0.5, mid="lavender",high = 'firebrick3', limits=c(0, 1)) +
#   labs(title="Rounded Predicted Turnout (1{p>=0.5})",
#        x ="Longitude", y = "Latitude", color = "Turnout") + 
#   theme_bw() +
#   theme(plot.title = element_text(hjust = 0.5))+ 
#   geom_point(data = points[c(30, 45, 60),], aes(x=longitude, y=latitude), shape = 15, color = "black", size = 2, show.legend = FALSE) +
...
p4 <- ggplot() +
  geom_point(data = pred_plot_coin, aes(x=X2, y=X1, color = pred_Y), size=0.8) +
  geom_path(aes(points$longitude, points$latitude), size=0.8) +
  scale_color_gradient2(low = 'dodgerblue3', midpoint=0.5, mid="lavender",high = 'firebrick3', limits=c(0, 1)) +
  labs(title="Coin Flip Based on Predicted Probability",
       x ="Longitude", y = "Latitude", color = "Turnout") + 
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))+ 
  geom_point(data = points[c(30, 45, 60),], aes(x=longitude, y=latitude), shape = 15, color = "black", size = 2, show.legend = FALSE) +
...
p5 <- ggplot() +
  ...
  geom_path(aes(points$longitude, points$latitude), size=0.8) +
  scale_color_gradient2(low = 'dodgerblue3', midpoint=0.5, mid="lavender",high = 'firebrick3', limits=c(0, 1)) +
  labs(title="Simulated Turnout (Coin Flip)",
       x ="Longitude", y = "Latitude", color = "Turnout") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) + 
  geom_point(data = points[c(30, 45, 60),], aes(x=longitude, y=latitude), shape = 15, color = "black", size = 2, show.legend = FALSE) +
