...
# Histogram of matched distances
#par(mfrow=c(4,2))
#pdf("histogram-buffers.pdf")
#for(i in 1:nbuff) hist(finalres[[i]][,"distance"], main = paste("Histogram of distance between matched pairs - Buffer", buffer.val[i], sep=""))
#dev.off()
...
names(finalbal) <-  c("buff1000","buff500")


for(j in 1:nbuff) {
  cat("Calculating post-matching balance testst for buffer", buffer.val[j], "\n")    
  results <- finalres[[j]]
  indx.tr <- results[,"indx.tr"]
  indx.co <- results[,"indx.co"]
  
  ## covariates we would like to have balance on 
  Btr <- data[indx.tr,c("salepr", "sqft_price", "lotarea", "bldglivingarea")]
  Bco <- data[indx.co,c("salepr", "sqft_price", "lotarea", "bldglivingarea")]  
  ...
  print(resbal)
   ...
   #finalbal = c(finalbal, list(resbal))
  finalbal[[j]] = resbal
}
