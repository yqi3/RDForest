...
lower_bound_default <- (1+ (1/3)*log(1-0.05) / log(0.05))^(-1)  # d=3, alpha=0.05
result_trivariate <- c() # vector to collect results
coverage_rf <- c()
BLB1 <- c()
BLB0 <- c()
...
for (i in 1:t) {
  ...
  if (default.forest.error < retrained.forest.error) {
    ...
    lower_bound <- (1+ (1/3)*log(1-a) / log(a))^(-1)  # d=3
  ...
  result_trivariate <- append(result_trivariate, prediction)
  ...
  try({print(i)
    ...
    print("----------------")
    print(paste0("Mean of RF:", mean(result_trivariate, na.rm = TRUE)))
    print("----------------")
    print(paste0("Var of RF: ", var(result_trivariate, na.rm = TRUE)))
    print(paste0("Var of RF (BLB): ", variance))
    print("----------------")
    print(paste0("MSE of RF: ", (mean(result_trivariate,na.rm = TRUE)-tau)^2+var(result_trivariate,na.rm = TRUE)))
    print("----------------")
