#### This file contains useful functions for the modeling part of the Keele & Titiunik (2015) section
set.seed(1234)
...
genDGP <- function(n, outcome){
  ...
  # Restrict range
  rand <- sample_n(data[between(data$latitude, min(pointsALL$latitude), max(pointsALL$latitude)) & between(data$longitude, min(pointsALL$longitude), max(pointsALL$longitude)), ], n, replace=TRUE)
  ...
  for (i in 1:n) {# Identify "region" based on longitude
    ...
    if ((closest_x <= 26) | (closest_x >= 44 & closest_x <= 49) | 
      ...
      # print(index1)
      # print(pointsALL$latitude[index1])
      # print(simdata$latitude[i])
