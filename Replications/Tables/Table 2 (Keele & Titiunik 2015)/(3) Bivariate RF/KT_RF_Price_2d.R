...
for (i in 1:t){
  ...
  forestT <- regression_forest(feature_trt, trt$price, mtry=1, num.trees=5000, tune.parameters = c("sample.fraction", "honesty.fraction", "alpha"))
  forestC <- regression_forest(feature_ctrl, ctrl$price, mtry=1, num.trees=5000, tune.parameters = c("sample.fraction", "honesty.fraction", "alpha"))
